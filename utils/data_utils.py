...
def ordering(x, pointers):
    new_x = torch.rand(x.size()).to('cuda')
    for i in range(pointers.size(0)):
      for pl in range(pointers.size(1)):
        #p_batch = pointers[i,pl]
        
        new_x[i,:,:,pl] = x[i,:,:,pointers[i,pl]]
                   
    return new_x

def randomic_order(x):
    
    new_x = torch.rand(x.size()).to('cuda')
    for i in range(x.size(0)):
        for pl in range(x.size(3)):
            order = torch.randperm(11)
            new_x[i,:,:,pl] = x[i,:,:,order[pl]]
